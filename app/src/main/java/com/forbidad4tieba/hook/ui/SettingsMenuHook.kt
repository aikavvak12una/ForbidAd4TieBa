...
object SettingsMenuHook {
    ...
    private val sSettingsFieldCache = ConcurrentHashMap<Class<*>, Field>()
    ...
    private fun showModuleSettingsDialog(context: Context, classLoader: ClassLoader?) {
        try {
            ...
            val switches = listOf(
                ...
                SwitchItem("屏蔽推荐进吧卡片", "block_my_forum", true, false),
                SwitchItem("精简进吧页面", "filter_enter_forum_web", true, false),
                SwitchItem("开启后台自动签到", "enable_auto_sign_in", true, false),
            )
            ...
            for (i in switches.indices) {
                ...
                root.addView(createSwitchRow(context, prefs, finalLabel, prefKey, padding, supported, item.defaultValue))
            }
    ...
    private fun createSwitchRow(
        ...
        val sw = Switch(context).apply {
            isChecked = if (enabled) prefs.getBoolean(prefKey, defaultValue) else false
            isEnabled = enabled
            setOnCheckedChangeListener { _, isChecked ->
                if (enabled) prefs.edit().putBoolean(prefKey, isChecked).apply()
            }
        }
        row.addView(sw)

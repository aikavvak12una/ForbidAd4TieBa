    ...
    fun tryAutoSignIn(context: Context, force: Boolean = false) {
        ...
        thread(isDaemon = true, name = "tbhook-auto-signin") {
            try {
                ...
                if (force) toast(context, "开始后台自动补签，共待签 $pendingCount 个吧...")
                ...
                XposedBridge.log("$TAG: 签到完毕。共尝试补签 $pendingCount 个吧，成功 $successCount 个")
                
                // 本轮请求过就视为签过了，不再循环无限轰炸死吧
                prefs.edit().putString(prefKey, today).apply()
                
                toast(context, "自动补签完成：总吧数 $totalLiked, 成功补签 $successCount 个")
            } catch (t: Throwable) {
